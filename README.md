...
├── templates/counter_l0/   счётчик без GPIO для STM32L0 (Cortex-M0+)
...
| Записать .bin/.hex/.elf и сверить | `mk.flash(image, device=...)` | `mk.stlink.flash(image, addr=...)` | **да** |
| Сбросить и пустить | `mk.reset_run()` | `mk.stlink.reset_run()` | нет |
...
риска нет (`mcukit.stlink` docstring).

**Важно для чтения через ST-Link**: обычный `connect` (умолчание CLI)
сам сбрасывает ядро — `info`/`read32`/`watch`/`save` в `mcukit.stlink`
подключаются в режиме `HOTPLUG` по умолчанию именно поэтому
...
Новый проект: `templates/blink_f4` (GD32/STM32F4, со светодиодами) или
`templates/counter_l0` (STM32L0, без единого обращения к GPIO — для
первой проверки записи на неизвестной плате). Поправить `link.ld`,
`project.py`, запустить `python run.py`.
...
* **ST-Link** — `STM32_Programmer_CLI.exe -c port=SWD ... -r32/-u/-d ...`,
  ...
  но по другой причине (`30-07`). Подключение без `mode=HOTPLUG` сбрасывает
  ядро (`30-08`) — для чтения, которое не должно тревожить код, это
...
Python 3.12: `probe`, `info`, `read32`, `watch`, `save`, `flash`,
`reset_run` — включая полный цикл на плате с рабочей прошивкой: `save()`
всего флеша, тестовая запись (без единого обращения к периферии,
`templates/counter_l0`) со сверкой и сбросом, доказательство по
счётчику, запись сохранённого дампа обратно, повторное чтение совпало
побайтово (`knowledge/40_СРЕДА/40-03`).
...
**Не проверено (ST-Link):** образы, отличные от `.bin`, для `flash()`;
чип с включённой защитой чтения; выбор зонда по `sn=` среди нескольких
(`30-07`); путь с кириллицей к образу; GD32 через ST-Link — не
предполагается (`20-04`); другие версии STM32CubeProgrammer.
